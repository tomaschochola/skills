...
Declare `.PHONY` and `.SILENT` directly above each target, never grouped in a shared list. Name phony goals lowercase snake_case, preferring one token; join two-word public names into one token and require minimum two tokens joined by underscore for protected names. Keep private targets file-based non-phony. Canonical targets: `fix` runs every defined automatic fixer, `check` runs the full possible local verify pipeline, `update` updates toolchains to the latest available stable release with automatic lockfile update.
